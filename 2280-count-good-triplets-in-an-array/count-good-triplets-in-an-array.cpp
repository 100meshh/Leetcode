class Solution {
    ...
    void updateSegTree(ll st_idx, ll start, ll end, ll& query_idx) {
        if (end < query_idx || start > query_idx) // No Overlap
            return;
    ...
    ll rangeSumQuery(ll st_idx, ll start, ll end, ll qs, ll qe) {
        if (qs > end || qe < start) return 0; // No overlap
        if (start >= qs && end <= qe) return seg_tree[st_idx]; // Total Overlap
